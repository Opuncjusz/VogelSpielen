...
public class Match {
    ...
    public static Match createFromMatchTO(MatchTO matchTO) {
        Match match = new Match();
        match.setId(matchTO.getId());
        match.setDesires(new ArrayList());
        for (DesireTO desireTO : matchTO.getDesires()) {
            match.getDesires().add(Desire.createFromDesireTO(desireTO.getStakeholder(), desireTO));
        }
        match.setFrom(matchTO.getFrom());
        match.setTo(matchTO.getTo());
        match.setPlace(matchTO.getPlace());
        match.setHasBeenSentNotification(false);
        return match;
    }
